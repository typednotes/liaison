...
/-- Link flags for a pkg-config package: its `--libs`, plus an explicit
    `-L<libdir>` from `--variable=libdir` (needed since `--libs` omits
    "default" library directories that Lean's bundled `ld.lld` doesn't
    search on its own). -/
...
-- (`__libc_csu_init` / GLIBC-version mismatches against Lean's bundled
-- glibc) and risks a wrong dylib binding on macOS. OpenSSL is needed at
-- *compile time only*, for `linen`'s own `ffi/jose.c` — which `linen`
-- builds, not `liaison` — so CI installs headers but `nativeLinkArgs`
-- carries none. `web-data/lakefile.lean` does pass OpenSSL link flags;
-- that looks like a latent issue there and is out of scope to fix here.
